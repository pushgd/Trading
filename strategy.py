...
def MACrossoverEntry(trade):
    ...
    if(Common.isUpwardPattern(last)):
        if(trade.symbol.top.info[Constant.KEY_CLOSE] > trade.symbol.top.info[Constant.KEY_MOVING_AVERAGE_SHORT_CLOSE] and last.info[Constant.KEY_HIGH] < last.info[Constant.KEY_MOVING_AVERAGE_SHORT_CLOSE] and trade.symbol.top.info[Constant.KEY_RSI] >= 55):
    ...
    if(trade.status == Constant.TRADE_ENTRY):
                if price > trade.takeProfit:
                    ...
                    print("Sell for Trade at " +
                          str(trade.tick.info[Constant.KEY_DATE])+" at Price "+str(price)+" for takeProfit "+str(trade.takeProfit)+" on "+trade.symbol.top.info[Constant.KEY_DATE])
                    return 1
    if(trade.status != Constant.TRADE_COMPLETED):
                if price < trade.stopLoss:
                    trade.status = Constant.TRADE_COMPLETED
                    print("Exit for Trade at " +
                          str(trade.tick.info[Constant.KEY_DATE])+" at Price "+str(price)+" for stoploss "+str(trade.stopLoss)+" on "+trade.symbol.top.info[Constant.KEY_DATE])
                    return 1
    return 0
...
def init():
    ...
    s = Strategy(Constant.STRATEGY_GANN_ANALYSIS,GannAnalysisCheckForEntry,GannAnalysisOnEnter,GannAnalysisCheckForExit)
...
def GannAnalysisCheckForExit(trade):
    if trade.symbol.lastTradedPrice > trade.buyTrigger:
        ...
        print("Sell for Trade at " +
                          str(trade.tick.info[Constant.KEY_DATE])+" at Price "+str(trade.symbol.lastTradedPrice)+" for takeProfit "+str(trade.takeProfit)+" on "+trade.symbol.top.info[Constant.KEY_DATE])
        return 1
    if trade.symbol.lastTradedPrice < trade.stopLoss:
        print("Exit for Trade at " +
                          str(trade.tick.info[Constant.KEY_DATE])+" at Price "+str(trade.symbol.lastTradedPric)+" for stoploss "+str(trade.stopLoss)+" on "+trade.symbol.top.info[Constant.KEY_DATE])
        return 1
    return 0
